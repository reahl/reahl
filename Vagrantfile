...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    apt-get update
    apt-get install -y python-virtualenv virtualenvwrapper python-dev gcc cython libxml2-dev libxslt-dev libsqlite3-0 sqlite3 postgresql-server-dev-all zlib1g-dev libfreetype6-dev equivs openssh-client dpkg-dev unzip python3-dev git postgresql-client-9.5 postgresql-9.5 
